...
void shell_sort_opt(int * a,int * dest, int N)
{
    dest = (int * )malloc(sizeof(int)*N);
    memcpy(dest,a,sizeof(int)*N);
    int i, j, k, h, v;
    for (h = N / 3; h > 0; h = (h-1)/3)
    {
        for (i = 0; i < h; i++)
        {
            for (j = i + h; j < N; j += h)
...
void shell_sort_ref(int * a, int N)
{
    int i, j, k, h, v;
    for (h = N / 2; h > 0; h = h/2)
    {
...
#define SIZE 27
int main(){
    int inum[SIZE]  ={3,5,6,1234,5,436,13,634,76,547,547,63,461,34,3,546,3234,6234,6,234,2,7,32,5,8,2,6};
    int input[SIZE] = {0,};

    memcpy(input, inum, SIZE*4);
    shell_sort_ref(input, SIZE);
